...
import com.tomboati.tour.view.activity.doa_dzikir.HitungTasbihActivity;
import com.tomboati.tour.view.activity.doa_dzikir.HitungTasbihLainnyaActivity;
...
public class TasbihAdapter extends RecyclerView.Adapter<TasbihAdapter.ViewHolder>{
    ...
    private final List<TasbihModel> models;
    ...
    @Override
    public void onBindViewHolder(@NonNull TasbihAdapter.ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        holder.nama_tasbih.setText(models.get(position).getJudul());
        holder.cardViewTasbih.setOnClickListener(v -> {
            if(position == models.size() - 1) {
                ...
                btnHitung.setOnClickListener(v12 -> {
                    String ayat = inpAyat.getText().toString();
                    String max = inpMax.getText().toString();
                    if(ayat.equals("") || max.equals("")) {
                        Toast.makeText(v12.getContext(), "Mohon isi field terlebih dahulu!" , Toast.LENGTH_SHORT).show();
                    } else if(Integer.parseInt(max) < 1) {
                        Toast.makeText(v12.getContext(), "Maksimal hitungan harus lebih dari 0" , Toast.LENGTH_SHORT).show();
                    } else {
                        Intent intent = new Intent(v12.getContext(), HitungTasbihLainnyaActivity.class);
